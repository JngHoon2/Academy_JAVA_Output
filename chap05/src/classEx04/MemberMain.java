...
public class MemberMain {
	...
	public static void main(String[] args) {
		...
		member[5] = "영주,20190509,20190510,2,여,40대후반";
		...
		showInfo(member, arrayMembers);
		
		System.out.println("20대후반 여성의 총방문횟수 : " + Female20Visit(arrayMembers));
		System.out.println("중장년층의 총 인원 수 : " + oldCnt(arrayMembers));
		
	}
	
	public static void showInfo(String[] member, ArrayList<Member> arrayMembers) {
		System.out.println("=================================================================");
		System.out.println("이름\t가입일\t\t최종방문일\t\t방문횟수\t성별\t연령대");
		System.out.println("=================================================================");
		for(int i = 0; i < member.length; i++) {
			arrayMembers.add(new Member(member[i].split(",")[0], member[i].split(",")[1], member[i].split(",")[2], 
					Integer.parseInt(member[i].split(",")[3]), member[i].split(",")[4].charAt(0), member[i].split(",")[5]));
			
			System.out.println(arrayMembers.get(i).getName() + "\t" + arrayMembers.get(i).getJoinDate() + "\t" + arrayMembers.get(i).getLastVisitDate() + "\t"
					+ arrayMembers.get(i).getNumVisit() + "\t" + arrayMembers.get(i).getGender() + "\t" + arrayMembers.get(i).getAge());			
		}
		System.out.println("=================================================================");
	}
	
	public static int Female20Visit(ArrayList<Member> arrayMembers) {
		int cnt = 0;
		
		for(Member m : arrayMembers) {
			if (m.getGender() == '여' && m.getAge().equals("20대후반")) {
				cnt += m.getNumVisit();
			}
		}
		return cnt;
	...
	public static int oldCnt(ArrayList<Member> arrayMembers) {
		int cnt = 0;
		
		for (Member m : arrayMembers) {
			if (m.getAge().contains("40대") || m.getAge().contains("50대") || m.getAge().contains("60대")) {
				cnt++;
			}
		}
