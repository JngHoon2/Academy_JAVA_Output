			...
			<table class="list">
					...
					<td align="center" colspan="9">
						<form action="${pageContext.request.contextPath}/productList.do" method="get">
							<p>
								...
								<input type="button" onclick="javascript:location.href='productList.do'" value="전체보기" />
								<c:if test="${not empty id}">
									<input type="button" onclick="javascript:location.href='productInsert.do'" value="상품등록" />
								</c:if>	
							</p>
						</form>				
					</td>
					...
					<td colspan="9">
						<div style="text-align: right;">
							<c:if test="${not empty id}" >
								...
								<a href="${pageContext.request.contextPath}/logout.do"><strong>로그아웃</strong></a>
							</c:if>
							<c:if test="${empty id}" >
								<a href="${pageContext.request.contextPath}/worderLogin"><strong>로그인</strong></a>
							</c:if>
						</div>				
				...
				<c:forEach var="product" items="${productList}">
					<tr class="record">
						...
						<td>
							<a href="productModify.do?code=${product.code }">상품수정</a>
						</td>
						<td>
