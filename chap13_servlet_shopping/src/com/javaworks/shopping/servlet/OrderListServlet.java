...
public class OrderListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("여기는 OrderListServlet의 doGet()메소드입니다.");
		String userId = "";
		...
		if(user == null || user.getUserId().equals("")) {	// 세션이 없으면 로그인 페이지로 이동
			//url = "/WEB-INF/views/user/login.jsp";		// 필터에서 체크해서 로그인되어 있지 않으면 강제로 로그인 페이지로 이동시킨다.
		}else {
			userId = user.getUserId();
		
			if(session.getAttribute("orderId") != null) {
				orderId = (int) session.getAttribute("orderId");
				System.out.println("세션에 저장한 orderId : " + orderId);
			...
			// DB작업을 위한 Dao 객체 생성
			orderDao = new OrderDao(DbConn.getConnection());
