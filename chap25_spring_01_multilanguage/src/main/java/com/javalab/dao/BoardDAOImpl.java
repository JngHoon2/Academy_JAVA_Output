...
public class BoardDAOImpl implements BoardDAO {
	...
	public int insertBoard(BoardVO vo) {
		...
		int result = 0;
		// 정상코드
		result = jdbcTemplate.update(BOARD_INSERT, vo.getTitle(), vo.getId(), vo.getContent());
		// 단위테스트를 위한 코드(no를 단위테스트에서 보낸 걸로 저장해야 함)
		//result = jdbcTemplate.update(BOARD_INSERT_TRANSACTION, vo.getNo(), vo.getTitle(), vo.getWriter(), vo.getContent());
		return result;
	} 
