...
public class LoginController {
	...
	/*
	 *  로그인 처리 메소드(핸들러)
	 *  @RequestParam("id") : 화면에서 name="id" 로 되어있는 요소의 값을 전달받음
	 */
	//@RequestMapping(value="/login/login.do", method = RequestMethod.POST)
	/*
	@PostMapping("/login.do")
	public String login(@RequestParam("id") String id,  // 화면에서 전달받음 
		...
		if(userVo != null) {
			...
			model.addAttribute("user", vo); // 화면에서 받은 아이디/비밀번호를 로그인폼에서 사용하도록 저장
			returnUrl = "redirect:" + "/login/login.do"; // 다시 로그인 폼으로 이동
		}
