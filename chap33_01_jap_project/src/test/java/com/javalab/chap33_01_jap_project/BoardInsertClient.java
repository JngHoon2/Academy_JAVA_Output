...
public class BoardInsertClient {
	...
	public static void main(String[] args) {
		...
		try {
			...
			board.setContent("JPA 관련 게시물의 내용힙니다.-1");
			...
			tx.commit(); // commit 실행 시 DB에 저장.
			System.out.println("commit 실행 직후-3");
			
			// [5] 글 목록 조회
			// [5.1] jpql 쿼리 생성, JPQL은 SQL 과 비슷한 문법을 가진 객체지향 쿼리.
			// Java Persistence Query Language
			// - from Board b : Board는 자바의 엔티티 클래스이며 b는 엔티티 클래스의 alias(별칭)임
			...
			// - 결과를 컬렉션으로 반환한다. 결과가 없으면 빈 컬렉션을 반환한다.
			// - 1건이면 1건만 들어간 컬렉션이 반환된다.
	        //  query.getSingleResult()
	        // - 결과가 정확히 1건 일때 사용
	        // - 결과가 없으면 NoResultException, 
	        // - 결과가 1건 이상이면 NonUniqueResultException 발생
			List<Board> boardList = board2.getResultList();
			// List<Board> boardList = em.createQuery(jpql, Board.class).getResultList(); 이런 식으로도 사용 가능하다.
