...
public class BoardReplyWriteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		System.out.println("BoardReplyWriteServlet doGet()");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		System.out.println("BoardReplyWriteServlet doPost()");
		...
		String id = "";		// 답글 작성자
		int reply_ref = 0;	// 원글  그룹번호
		int reply_lev = 0;	// 원글 레벨
		int reply_seq = 0;	// 원글 순서
		...
		if(request.getParameter("reply_ref") != null) {
			reply_ref = Integer.parseInt(request.getParameter("reply_ref"));
		}
		if(request.getParameter("reply_lev") != null) {
			reply_lev = Integer.parseInt(request.getParameter("reply_lev"));
		}
		if(request.getParameter("reply_seq") != null) {
			reply_seq = Integer.parseInt(request.getParameter("reply_seq"));
		}
		...
		board.setId(id);
		board.setReply_ref(reply_ref);
		board.setReply_lev(reply_lev + 1);
		board.setReply_seq(reply_seq + 1);
		
		// 전달할 객체가 제대로 생성되었지 검증
		System.out.println(board.toString());
		
		// 답글 저장전 사전 작업이 성공하면
		if(boardDao.reqUpdate(reply_ref, reply_seq)) {
			// 답글 저장 
